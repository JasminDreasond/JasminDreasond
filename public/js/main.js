import { generateRainbowText } from './rainbow.js';
import { initSkyCanvas } from './skyCanvas.js';
...
if (canvasElement) {
  skyController = initSkyCanvas(canvasElement);
}
...
const setupTimezone = () => {
  ...
  /** @type {number} */
  const age = Math.abs(new Date(monthDiff).getUTCFullYear() - 1970);

  document.getElementById('age').innerText = age.toString();

  const timezoneUpdate = () => {
    document.getElementById('timezone').innerText = moment
